...
# Adapted From: https://github.com/rajshah4/NBA_SportVu/blob/master/Extract_Three_Point_Plays.R
...
# Grabs play-by-play from JSON file
...
##Filters data for when the ball is 8 feet or higher, and 
# Adds two new variables: threedist (distance from ball to basket, xy only)
# and threedistz (distance from ball to basket, xyz)

# Removes duplicates (not sure why there are duplicates)
...
#final <- bind_rows(df_total) %>%
#  arrange(gameid, playid, desc(game_clock)) %>%
#  arrange(gameid, playid, desc(game_clock))

write.csv(df_startshot, "threePointShots0021500492.csv", row.names = FALSE)
write.csv(df_total, "threePointPlays0021500492.csv", row.names = FALSE)
